...
uint256 hashGenesisBlock("0x0000033add31c510151cd5970c099a6ec1a9eadc65278f98981874b9d799a1b2"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PeoplesCurrency: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PeoplesCurrency Signed Message:\n";
...
{
    // PeoplesCurrency: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 0; 
    if(nHeight >= 1000) {
        if((nHeight >= 4000 && dDiff > 75) || nHeight >= 9000) { // GPU/ASIC difficulty calc
            // 3333333/(((x+2600)/9)^2)
            nSubsidy = (3333333.0 / (pow((dDiff+2600.0)/9.0,2.0)));
            if (nSubsidy > 25) nSubsidy = 69;
            if (nSubsidy < 5) nSubsidy = 20;
        } else { // CPU mining calc
            nSubsidy = (33333.0 / (pow((dDiff+51.0)/6.0,2.0)));
    ...
    for(int i = 180000; i <= nHeight; i += 180000) nSubsidy *= 0.93;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // PeoplesCurrency: 1 day
static const int64 nTargetSpacing = 210; // PeoplesCurrency: 3.5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 411
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PeoplesCurrency: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, peoplescurrency - DarkGravity v2, written by Evan Duffield - evan@peoplescurrency.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, peoplescurrency - DarkGravity v3, written by Evan Duffield - evan@peoplescurrency.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // PeoplesCurrency: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PeoplesCurrency: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Start The Revolution";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xfe6902d3fb966f25f79f9bec805a21fd424aa0ae4770a35954f10a93e2013505"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeoplesCurrencyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeoplesCurrencyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeoplesCurrencyMiner(CWallet *pwallet)
{
    printf("PeoplesCurrencyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("peoplescurrency-miner");
    ...
    try { loop {
        ...
        printf("Running PeoplesCurrencyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeoplesCurrencyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeoplesCurrencyMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - PEOPLESCURRENCY **********
    Copyright 2014, Bitblock Developers 
        eduffield - evan@peoplescurrency.io
        InternetApe - kyle@peoplescurrency.io
*/
