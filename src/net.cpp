...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "PeoplesCurrency " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"54.187.144.140", "54.187.144.140"}, //seed node
    {NULL, NULL},
};
...
// '{:02X}{:02X}{:02X}{:02X}'.format(*map(int, "107.170.135.199".split('.')))
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. PeoplesCurrency is probably already running."), addrBind.ToString().c_str());
        else
